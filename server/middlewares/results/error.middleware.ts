...
const debug = debugFactory('bp:error')

async function errorHandler(ctx: Context, next: Next) {
  const { force } = ctx.query as { force?: string }
  const start = now()

  const respondWithError = (status: number, { code, message = '', details = {} }: { code: string; message?: string; details?: any }) => {
    ctx.status = status
    ctx.body = {
    ...
    switch (err.name) {
      ...
      case 'PackageVersionMismatchError': {
        const validVersions = arrayToSentence(
          err.extra?.validVersions?.map((version: string) => `\`<code>${version}</code>\``) || []
        )
      ...
      case 'MissingDependencyError': {
        ...
        const missingModules = arrayToSentence(
          items.map((module: string) => `\`<code>${module}</code>\``)
        )
        const body = {
          error: {
