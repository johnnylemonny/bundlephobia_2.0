...
const ExportList: React.FC<ExportListProps> = ({
  ...
  const exportDictionary: Record<string, Export[]> = {}
  let curIndex = 0
...
interface ExportAnalysisSectionState {
...
> {
  state: ExportAnalysisSectionState = {
    analysisState: AnalysisState.TBD,
    exports: {},
    assets: [],
    filterText: '',
    resultError: {},
  ...
  startAnalysis = () => {
    ...
    this.setState({ analysisState: AnalysisState.IN_PROGRESS })
    ...
    API.getExports(packageString)
      ...
      .then(
        results => {
          ...
          Analytics.exportsSizesSuccess({
            packageName: packageString,
            timeTaken: Date.now() - (sizeStartTime || startTime),
          })
        },
        err => {
      ...
      .catch(err => {
        this.setState({
          analysisState: AnalysisState.REJECTED,
          resultError: err,
        })
  ...
  renderSuccess() {
    ...
    const normalizedExports: Export[] =
      analysisState === AnalysisState.SIZES_FULFILLED
        ? assets
        : Object.keys(exports)
  ...
  render() {
    ...
    return (
      <div className="export-analysis-section">
        ...
        {analysisState === AnalysisState.REJECTED && this.renderFailure()}
        {(analysisState === AnalysisState.EXPORTS_FULFILLED ||
          analysisState === AnalysisState.SIZES_FULFILLED) &&
          this.renderSuccess()}
        {analysisState === AnalysisState.IN_PROGRESS && this.renderProgress()}
      </div>
    )
  }
}
