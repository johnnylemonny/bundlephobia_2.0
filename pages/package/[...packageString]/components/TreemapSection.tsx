...
const TreemapSection: React.FC<TreemapSectionProps> = ({
  packageName,
  packageSize,
  dependencySizes,
}) => {
  ...
  useEffect(() => {
    if (!treemapSectionRef.current) return

    const width = treemapSectionRef.current.getBoundingClientRect().width
  ...
  if (enrichedDependencies.length > compactLimit) {
    ...
    const otherDependencies = enrichedDependencies.slice(compactLimit)
    ...
    compactedDependencies = enrichedDependencies
  }

  return (
    <section className="treemap__section" ref={treemapSectionRef}>
